    ...
    btc_config_init(void)
{
    ...
    if (!config) {
        {if (2 >= 2 && (((2) > (3) ? (2) : (3)) >= 2)) {esp_log_write(ESP_LOG_WARN, "BT_BTC", "\033[0;" "33" "m" "W" " (%d) %s: " "%s unable to load config file; starting unconfigured.\n" "\033[0m" "\n", esp_log_timestamp(), "BT_BTC", __func__); };};
        config = config_new_empty();
        if (!config) {
            {if (2 >= 1 && (((2) > (3) ? (2) : (3)) >= 1)) {esp_log_write(ESP_LOG_ERROR, "BT_BTC", "\033[0;" "31" "m" "E" " (%d) %s: " "%s unable to allocate a config object.\n" "\033[0m" "\n", esp_log_timestamp(), "BT_BTC", __func__); };};
            goto error;
        }
    }
